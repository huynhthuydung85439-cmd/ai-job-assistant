...
AI 求职助手是一个基于 React、FastAPI、LangChain 和 DeepSeek 的全栈求职工作台。用户可以上传 PDF 简历，与岗位 JD 做结构化匹配，也可以进行 AI 对话，并把岗位和面试资料构建成个人 RAG 知识库。系统通过 JWT 认证、MySQL 业务数据过滤和 Chroma 元数据过滤实现用户隔离，并使用 Docker Compose 编排 Web、API、MySQL、Redis 及持久化卷。v0.1.0 已正式发布，自动化测试结果为 31 passed。
...
外部模型和向量依赖通过服务边界替换为测试实现，覆盖健康检查、认证持久化、PDF 解析、简历分析、对话、RAG、历史记录和用户隔离。v0.1.0 的测试基线为 `31 passed`。
...
可以现场展示 FastAPI 文档、MySQL 历史记录、Chroma 来源返回、账号切换后的数据隔离、容器重启后的持久化，以及 `pytest -q` 的 31 项通过结果。

## 9. 收尾表达

项目的重点不只是调用大模型，而是把模型能力放进一条可认证、可持久化、可检索、可回看、可部署和可测试的完整业务链路。v0.1.0 优先保证闭环和工程真实性，并明确保留后续扩展边界。
